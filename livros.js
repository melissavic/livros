...
const getLivro = function(nomeLivros){
    ...
    livros.forEach(item => {
        ...
        item.books.forEach(itemBooks => {
            if (itemBooks.title.toLowerCase().indexOf(nome.toLowerCase()) > -1) {
                listaLivros.push({titulo: itemBooks.title, descricao: itemBooks.subtitle, preco: itemBooks.price, foto: itemBooks.image})
                erro = false
            }
        })
    });

    json.total = listaLivros.length
